...
import { ClipboardCopyIcon, ExternalIcon, GlobeIcon, SearchIcon, XIcon } from '@oxide/assets'
import type { Category, GameVersion, Platform, ProjectType, SortType, Tags } from '@oxide/ui'
import {
    Button,
    Checkbox,
    defineMessages,
...
// CurseForge has a max of 50 results per page
...
type SearchResults = {
	total_hits: number
	limit: number
	hits: (SearchResult | NormalizedSearchResult)[]
}
...
// Map Modrinth sort types to CurseForge sort fields
function mapSortTypeToCurseForge(sortName: string): ModsSearchSortField {
	switch (sortName) {
		case 'relevance':
			return 'Featured'
		case 'downloads':
...
// Get active loader from filters
function getActiveLoader(): string | null {
	const loaderFilter = currentFilters.value.find((f) => f.type === 'mod_loader')
	return loaderFilter?.option ?? null
}

// Get active game version from filters
function getActiveGameVersion(): string | null {
	const versionFilter = currentFilters.value.find((f) => f.type === 'game_version')
	return versionFilter?.option ?? null
}

watch([requestParams, provider, () => route.params.projectType], () => {
	if (!route.params.projectType) return
	// Reset maxResults to 50 if switching to CurseForge with 100 selected
	if (isCurseForge.value && maxResults.value > 50) {
		maxResults.value = 50
	}
	refreshSearch()
})

async function refreshModrinthSearch() {
...
async function refreshCurseForgeSearch() {
	const currentProjectType = route.params.projectType as string
	const classId = projectTypeToClassId(currentProjectType)
	if (!classId) {
		return { hits: [], total_hits: 0, limit: maxResults.value }
	}

	const activeLoader = getActiveLoader()
	const activeGameVersion = getActiveGameVersion()

	const params: ModSearchParams = {
		class_id: classId,
		search_filter: query.value || undefined,
		game_version: activeGameVersion || undefined,
		mod_loader_type: activeLoader ? loaderToModLoaderType(activeLoader) : undefined,
		sort_field: mapSortTypeToCurseForge(currentSortType.value.name),
		sort_order: 'desc',
		index: (currentPage.value - 1) * maxResults.value,
		...
		const normalizedHits = cfResults.results.map((project) => {
			...
			if (instance.value && instanceProjects.value) {
				normalized.installed = Object.values(instanceProjects.value).some(
					(x) => x.metadata && x.metadata.project_id === normalized.id,
				)
			}
			return normalized
		})
...
async function refreshSearch() {
	...
	if (isModrinth.value) {
		searchResults = await refreshModrinthSearch()
	} else {
		searchResults = await refreshCurseForgeSearch()
	}
...
const messages = defineMessages({
	...
	openInProvider: {
		id: 'browse.context.open-in-provider',
	...
	offlineModrinth: {
		id: 'browse.offline.modrinth',
		defaultMessage: 'You are currently offline. Connect to the internet to browse Modrinth!',
	},
	offlineCurseforge: {
		id: 'browse.offline.curseforge',
		defaultMessage: 'You are currently offline. Connect to the internet to browse CurseForge!',
	},
})
...
// Get the URL for a search result based on its source
function getResultUrl(result) {
	// Check if it's a normalized result (from CurseForge)
	if (result.source) {
		return getProjectUrl(result)
	}
	// Modrinth result
...
const handleOptionsClick = (args) => {
	const url = getResultUrl(args.item)
	switch (args.option) {
		case 'open_link':
			openUrl(url)
			break
	...
	<div ref="searchWrapper" class="flex flex-col gap-3 p-6">
		...
		<div class="search">
			...
			<section v-else-if="offline && results.total_hits === 0" class="offline">
				{{
					isModrinth
						? formatMessage(messages.offlineModrinth)
						: formatMessage(messages.offlineCurseforge)
				}}
			</section>
			<section v-else class="project-list display-mode--list instance-results" role="list">
				<SearchCard
					v-for="result in results.hits"
					:key="result?.project_id ?? result?.id"
					:project="result"
					:instance="instance"
					:categories="
						isModrinth
					...
					:installed="
						result.installed ||
						newlyInstalled.includes(result.project_id) ||
						newlyInstalled.includes(result.id)
					"
					@install="
						(id) => {
				...
				<ContextMenu ref="options" @option-clicked="handleOptionsClick">
					<template #open_link>
						<GlobeIcon />
						{{ formatMessage(messages.openInProvider, { provider: isModrinth ? 'Modrinth' : 'CurseForge' }) }}
						<ExternalIcon />
					</template>
					<template #copy_link>
						<ClipboardCopyIcon />
