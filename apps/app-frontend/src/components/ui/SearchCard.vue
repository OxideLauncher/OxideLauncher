<template>
	...
	>
		<div class="icon w-[96px] h-[96px] relative">
			...
			<div
				v-if="project.source"
				class="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-bg flex items-center justify-center shadow-sm"
				:title="project.source === 'curseforge' ? 'CurseForge' : 'Modrinth'"
			>
				<FlameColorIcon v-if="project.source === 'curseforge'" class="w-3.5 h-3.5" />
				<ModrinthColorIcon v-else class="w-3.5 h-3.5" />
			</div>
		</div>
...
import { Avatar, ButtonStyled, injectNotificationManager } from '@oxide/ui'
...
function handleCardClick() {
	...
	if (props.project.source === 'curseforge') {
		// CurseForge projects open in app
		const cfData = props.project.curseforge_data
		if (cfData) {
			router.push({
				path: `/cf-project/${cfData.id}`,
				query: { i: props.instance ? props.instance.path : undefined },
			})
		}
	} else {
		// Modrinth projects open in app
		router.push({
