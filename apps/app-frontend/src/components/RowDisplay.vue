...
import { HeadingLink, injectNotificationManager } from '@oxide/ui'
...
function getCurseForgeClassSlug(projectType) {
	switch (projectType) {
		case 'mod':
			return 'mc-mods'
		case 'modpack':
...
const handleOptionsClick = async (args) => {
	switch (args.option) {
		...
		case 'open_link':
			if (args.item.source === 'curseforge' && args.item.curseforge_data) {
				const cfSlug = getCurseForgeClassSlug(args.item.project_type)
				openUrl(`https://www.curseforge.com/minecraft/${cfSlug}/${args.item.slug}`)
			} else {
		...
		case 'copy_link':
			if (args.item.source === 'curseforge' && args.item.curseforge_data) {
				...
				await navigator.clipboard.writeText(
					`https://www.curseforge.com/minecraft/${cfSlug}/${args.item.slug}`,
				)
			} else {
				await navigator.clipboard.writeText(
