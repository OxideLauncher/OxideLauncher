...
A flexible, type-safe API client for Modrinth's Labrinth API. Works across Node.js, browsers, Nuxt, and Tauri with a feature system for authentication, retries, circuit breaking and other custom processing of requests and responses.

## Installation

```bash
pnpm add @oxide/api-client
# or
npm install @oxide/api-client
# or
yarn add @oxide/api-client
```
...
```typescript
import { GenericModrinthClient, AuthFeature, ProjectV2 } from '@oxide/api-client'
...
```typescript
import { NuxtModrinthClient, AuthFeature, NuxtCircuitBreakerStorage } from '@oxide/api-client'
...
```typescript
import { TauriModrinthClient, AuthFeature } from '@oxide/api-client'
...
By default, the client uses the production base URL:

- `labrinthBaseUrl`: `https://api.modrinth.com/` (Labrinth API)

You can override this for staging environments or custom instances:
...
// Now requests will use the staging URL
